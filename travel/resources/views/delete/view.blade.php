        ...
        <div class="container itit-container">
            ...
            @if(Auth::check() && $itinerary->user_id == Auth::user()->id)
                ...
                @if($itinerary->published == env("ITI_PUBLISHED"))
                    <a type="button" class="btn btn-info text-center" href="{{route('itinerary.unpublish',[$itinerary->slug])}}">
                        Unpublish and Edit
                    </a>
                @else
                    {{-- cannot publich if iti doesnt have even 1 day details--}}
                    @if($itinerary->days->count() >= 1)
                        <a type="button" class="btn btn-info text-center" href="{{route('itinerary.publish',[$itinerary->slug])}}">
                            Publish
                        </a>
            ...
            <div class="text-center top-buffer">
                ...
                @if($is_preview)
                    @if($itinerary->price == 0)
                        <a type="button" class="itit-button itit-footer-button btn-primary" href="{{ route('itinerary.getItitFree', $itinerary->slug) }}">Get This</a>
                    @else
                        <a type="button" class="itit-button itit-footer-button btn-primary" href="{{ route('itinerary.purchaseConfirm', $itinerary->slug) }}">BUY FULL ITINERARY</a>
                    @endif
